            ...
            @else
                @foreach ($borrowings as $borrowing)
                    <div class="bg-white p-4 rounded-lg shadow-md border border-gray-200">
                        <p class="font-bold text-lg">{{ $borrowing->book ? $borrowing->book->title : '📌 Livre supprimé' }}</p>
                        <p class="text-sm text-gray-600">👤 Emprunté par : <span class="font-medium">{{ $borrowing->user->name }}</span></p>
                        <p class="text-sm text-gray-600">📅 Date d'emprunt : <span class="font-medium">{{ \Carbon\Carbon::parse($borrowing->borrowed_at)->format('d/m/Y') }}</span></p>
                        <p class="text-sm text-gray-600">🔄 Retour prévu : <span class="font-medium">{{ \Carbon\Carbon::parse($borrowing->due_date)->format('d/m/Y') }}</span></p>
